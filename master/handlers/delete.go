...
func DeleteHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err := database.DeleteRecord(req); err != nil {
		writeJSON(w, http.StatusInternalServerError, false, err.Error())
		return
	}

	if err := replication.ReplicateDelete(req); err != nil {
		writeJSON(w, http.StatusInternalServerError, false, "record deleted locally but replication failed")
		return
	}
